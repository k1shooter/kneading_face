...
class FacialExpressionApp {
  ...
  handleFile(file) {
    ...
    const selectedExpression = document.querySelector(
      '.expression-card.selected'
    );
    if (!selectedExpression) {
      const defaultExpression = document.querySelector(
        '.expression-card[data-expression="happy"]'
      );
  ...
  showExpressionDetails(expression, description) {
    ...
    if (detailsElement) {
      detailsElement.innerHTML = `
                <h4>Selected Expression: ${
                  expression.charAt(0).toUpperCase() + expression.slice(1)
                }</h4>
                <p>${
                  description || 'Transform your image with this expression.'
                }</p>
            `;
  ...
  updateTransformButton() {
    ...
    if (transformBtn) {
      ...
      transformBtn.textContent = canTransform
        ? 'Transform Image'
        : 'Select Image & Expression';
    }
  }
    ...
    this.pollingInterval = setInterval(async () => {
      ...
      try {
        const response = await fetch(
          `${this.config.apiEndpoints.status}${this.currentConversionId}`
        );
        ...
        if (status.status === 'completed') {
          ...
          this.handleTransformationError(
            status.error || 'Transformation timed out'
          );
          this.stopStatusPolling();
        }
      } catch (error) {
  ...
  displayResults(status) {
    ...
    if (transformedImage && status.result_url) {
      ...
      transformedImage.dataset.downloadUrl =
        status.download_url || status.result_url;
    }
        ...
        (item) => `
            <div class="history-item" data-id="${item.id}">
                <div class="history-thumbnail">
                    <img src="${item.thumbnail_url}" alt="Conversion ${
          item.id
        }" loading="lazy">
                </div>
          ...
          item.status
                ...
                <div class="history-actions">
                    <button class="btn-small" onclick="app.viewHistoryItem('${
                      item.id
                    }')">View</button>
                    <button class="btn-small" onclick="app.downloadHistoryItem('${
                      item.id
                    }')">Download</button>
                </div>
            </div>
